...
The package ships two agent skills (kept in `skills/` for reference, not registered with pi to avoid conflicts with `~/.agents/skills`):
...
- `usage_tracker`: remaining subscription quota (used % / left % / reset) for providers linked in pi. Answers "how much of my plan is left".
...
- `/usage [refresh|hide]`: remaining subscription quota for linked providers. Footer stays on; the command opens a widget with used/left/reset per window. `/usage hide` closes it.
