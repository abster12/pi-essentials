 ...
 *
 * Ported from mitsuhiko/agent-stuff. Schema uses Type.Union instead of StringEnum.
 * Summarization uses complete() from @earendil-works/pi-ai/compat. Clear uses SessionManager.branch.
 */
...
import { type Model, type Api, type UserMessage, type TextContent } from "@earendil-works/pi-ai";
...
export function wrapSenderInfo(sessionId?: string, sessionName?: string): string {
	if (!sessionId) return "";
...
async function handleCommand(
	...
	if (command.type === "clear") {
		...
		// SessionManager is read-only on ExtensionContext; branch() is the current rewind API.
		try {
...
async function maybeHandleStartupControlSend(pi: ExtensionAPI, ctx: ExtensionContext): Promise<void> {
	...
	const senderInfo = includeSenderInfo
		? wrapSenderInfo(ctx.sessionManager.getSessionId(), ctx.sessionManager.getSessionName()?.trim())
		: "";
