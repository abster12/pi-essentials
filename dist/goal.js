...
var STATE_TYPE = "goal";
var UI_MESSAGE_TYPE = "goal-ui";
var CONTINUATION_MESSAGE_TYPE = "goal-continuation";
var MAX_OBJECTIVE_CHARS = 4e3;
var CreateGoalParams = Type.Object({
  objective: Type.String({
    description: "Required. The concrete objective to start pursuing. This starts a new active goal when no unfinished goal exists. If the previous goal is complete, it is replaced."
  }),
...
function normalizeGoal(value) {
  ...
  const tokenBudget = typeof raw.tokenBudget === "number" && Number.isFinite(raw.tokenBudget) && raw.tokenBudget > 0 ? Math.floor(raw.tokenBudget) : void 0;
  const ts = nowSeconds();
  return {
...
function assistantUsageTokens(messages) {
  let total = 0;
...
function goalResponse(goal, sessionId, includeCompletionReport = false) {
  const wireGoal = goal ? {
    threadId: sessionId,
  ...
  const remainingTokens = goal?.tokenBudget === void 0 ? null : Math.max(0, goal.tokenBudget - goal.tokensUsed);
  let completionBudgetReport = null;
...
function continuationPrompt(goal) {
  const tokenBudget = goal.tokenBudget === void 0 ? "none" : String(goal.tokenBudget);
  const remainingTokens = goal.tokenBudget === void 0 ? "unbounded" : String(Math.max(0, goal.tokenBudget - goal.tokensUsed));
...
Time spent pursuing goal: ${goal.timeUsedSeconds} seconds
Tokens used: ${goal.tokensUsed}
Token budget: ${goal.tokenBudget === void 0 ? "none" : goal.tokenBudget}
Tokens remaining: ${goal.tokenBudget === void 0 ? "unbounded" : Math.max(0, goal.tokenBudget - goal.tokensUsed)}
...
function budgetLimitMessage(goal) {
...
function statusAfterObjectiveEdit(status) {
  switch (status) {
    case "complete":
    case "budgetLimited":
      return "active";
...
function goalExtension(pi) {
  ...
  function updateStatus(ctx) {
    ...
    switch (goal.status) {
      case "active": {
        ...
        const usage = snapshot.tokenBudget === void 0 ? ` (${formatElapsedSeconds(snapshot.timeUsedSeconds)})` : ` (${formatTokensCompact(snapshot.tokensUsed)} / ${formatTokensCompact(snapshot.tokenBudget)})`;
  ...
  function showGoalMessage(content) {
    pi.sendMessage(
      {
  ...
  function setGoal(objectiveInput, tokenBudgetInput) {
  ...
  function editGoalObjective(objectiveInput) {
    if (!goal) {
  ...
  function maybeApplyBudgetLimit() {
    if (!goal || goal.status !== "active" || goal.tokenBudget === void 0) return false;
...
${goalSummary(goal)}`);
  ...
  pi.registerTool({
    name: "create_goal",
    label: "Create Goal",
    description: "Create a goal only when explicitly requested by the user or system/developer instructions; do not infer goals from ordinary tasks. Set token_budget only when an explicit token budget is requested. Fails if an unfinished goal exists; if the previous goal is complete, it is replaced.",
